...
async def handle_complete_task(payload: dict[str, Any], db: AsyncSession) -> str:
    ...
    tasks = await task_service.get_tasks(db, include_completed=False)
    match = next((t for t in tasks if title in t.title.lower()), None)
    if match is None:
        return f"❌ No open task matching \"{payload.get('title')}\" found."

    from memocore.schemas.task import TaskUpdate
    await task_service.update_task(db, match.id, TaskUpdate(is_completed=True))
    return f"✅ Marked **{match.title}** as complete!"
...
async def handle_delete_task(payload: dict[str, Any], db: AsyncSession) -> str:
    ...
    match = next((t for t in tasks if title in t.title.lower()), None)
    if match is None:
        return f"❌ No task matching \"{payload.get('title')}\" found."

    await task_service.delete_task(db, match.id)
    return f"🗑️ Task **{match.title}** deleted."


async def handle_delete_all_tasks(payload: dict[str, Any], db: AsyncSession) -> str:
    tasks = await task_service.get_tasks(db, include_completed=False)
    if not tasks:
        return "📭 You have no open tasks to delete."

    for task in tasks:
        await task_service.delete_task(db, task.id)
    return f"🗑️ Deleted all {len(tasks)} open task(s)."
